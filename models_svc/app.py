...
def score(features: dict):
    # Bot-context proxy (extended with flags)
    bot_ctx = 0.0
    ...
    # Human-motoric: higher is more human
    ...
    return {
        'scores': {
            'bot_context': round(bot_ctx,3),
            'human_motoric': round(human_motoric,3),
            'contextual_risk': round(ctx,3)
        },
